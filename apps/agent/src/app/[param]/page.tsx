...
import { TemplateAria } from '@repo/templates/aria';
...
type Templates = "astra" | "nest";

interface ParamPageProps {
  params: { param: Templates };
}

export default function Page({ params }: ParamPageProps): JSX.Element {
  const { param } = params;
  
  const [selectedTemplate, setSelectedTemplate] = useState<Templates>(param);

  const changeTemplate = (value: Templates): void => {
  ...
  return (
    <>
      {selectedTemplate === 'astra' ? (
        <TemplateAria onChangeTemplate={changeTemplate.bind(null, 'nest')} />
      ) : (
        <main className="flex flex-col items-center justify-between min-h-screen p-24">
          <div className="z-10 items-center justify-between w-full max-w-5xl font-mono text-sm lg:flex">
            <div className="flex flex-col space-y-2">
              ...
              <button className="fixed top-0 left-0 flex justify-center w-full px-4 pt-8 pb-6 border-b bg-gradient-to-b backdrop-blur-2xl border-neutral-800 bg-zinc-800/30 from-inherit lg:static lg:w-auto lg:rounded-xl lg:border lg:bg-gray-200 lg:p-4 lg:bg-zinc-800/30 hover:bg-[#f2f2f2] dark:hover:bg-[#1a1a1a] hover:border-transparent" onClick={changeTemplate.bind(null, 'astra')} type="button">
                Aria Template
              </button>
            </div>
